...
### Modo calibración automático (recomendado)

En lugar de calibrar a mano, activa el **modo calibración** en `config.h`:

| Variable              | Default | Descripción                                              |
|-----------------------|---------|----------------------------------------------------------|
| `CALIBRATION_MODE`    | `0`     | `1` = modo calibración, `0` = operación normal           |
| `CALIBRATION_SAMPLE_MS` | `5000` | Intervalo de muestreo automático en calibración (ms)    |

Con `CALIBRATION_MODE 1` el firmware:
- imprime en el Monitor Serie cada `CALIBRATION_SAMPLE_MS` (5 s por defecto):
  ```
  [CALIBRACION] Raw:  571 | Humedad:   0.0% | Min:  336 | Max:  571
  ```
- muestra el rango **Min/Max acumulado** del RAW, ideal para mover el
  sensor de aire→agua y capturar ambos extremos sin anotar a mano.
- mantiene el **relé desactivado** (sin riego) y **no publica telemetría MQTT**
  (la presencia online sí se publica). El servidor web local sigue activo.

**Procedimiento:**
1. Pon `CALIBRATION_MODE 1` y sube el firmware.
2. Abre el Monitor Serie (115200 baud). Verás el banner de instrucciones.
3. Deja el sensor **en el aire** unos segundos → el **Max** mostrado será tu `RAW_DRY`.
4. Sumerge el sensor **en agua** unos segundos → el **Min** mostrado será tu `RAW_WET`.
5. Escribe esos valores en `RAW_DRY` / `RAW_WET` y vuelve a poner
   `CALIBRATION_MODE 0` antes de operar normalmente.

> ⚠ En el ESP8266 el ADC es de **10 bits (0–1023)**, mientras que en el
> ESP32 es de 12 bits (0–4095). Si importas calibraciones del firmware
> ESP32, recalibra en el ESP8266: unos valores 12-bit en un ADC 10-bit
> provocan que la humedad quede fija en 100%.
...
**Formato del mensaje (canónico, igual que el firmware ESP32):**
```json
{
  "soil_humidity":         {"raw": 512, "unit": "%", "value": 51.2},
  "state":                 "WET",
  "watering":              false,
  "last_watered_sec":      120,
  "on_threshold_soil_vwc": 35,
  "relay_on_time_s":       1.0,
  "ambient_temperature":   {"unit": "°C", "value": 25.3},
  "ambient_humidity":      {"unit": "%", "value": 60.1}
}
```

- `soil_humidity` → humedad de suelo en % (objeto anidado con `raw`/`unit`/`value`).
- `on_threshold_soil_vwc` → umbral (%) que activa el riego.
- `ambient_temperature` / `ambient_humidity` → solo si `ENABLE_AMBIENT_SENSOR`
  está habilitado y la lectura del DHT es válida.

Valores posibles de `state`: `WET`, `DRY`, `WATERING`, `COOLDOWN`.

> Estos nombres coinciden con los campos que el dashboard del servidor
> (`em_server/app.py`) espera, por lo que no se requiere `field_mappings` en
> `config.json` para que la humedad, el umbral y la atmósfera se muestren.
