/*
  ...
  ───────────────────────
    ...
    4. Selecciona la placa ESP8266, compila y sube.
  ...
  ──────────────────────────────
    PUBLICACIÓN (Dispositivo → Raspberry Pi):
      ...
      Formato: {"soil_humidity":{"raw":512,"unit":"%","value":65.3},
                "state":"WET","watering":false,
                "on_threshold_soil_vwc":35,"relay_on_time_s":1.0}
...
#if CALIBRATION_MODE
// Estado del modo calibración: muestreo automático cada
// CALIBRATION_SAMPLE_MS con seguimiento del rango Min/Max del RAW.
// Se usa para ajustar RAW_DRY (sensor en aire = RAW máximo) y
// RAW_WET (sensor en agua = RAW mínimo).
unsigned long lastCalibrationSampleMs = 0;
int calibMinRaw = 1024;  // valor mínimo visto hasta ahora (1024 > ADC 10 bits)
int calibMaxRaw = -1;    // valor máximo visto hasta ahora
#endif
...
#if CALIBRATION_MODE
  // En modo calibración el riego se mantiene desactivado: nunca se abre
  // la electroválvula y el LED refleja el estado inactivo.
  (void)pct;
...
// ── buildJsonCore(percent, raw)  —  Payload JSON canónico ────────
// Construye el JSON compartido por el servidor web local (/json) y MQTT.
// Usa los nombres de campo canónicos que el servidor EM_server guarda y muestra
// sin necesidad de field_mappings:
//
//   {
//     "soil_humidity":         {"raw":512,"unit":"%","value":42.3},
//     "watering":              false,
//     "state":                 "WET",
//     "last_watered_sec":      120,
//     "on_threshold_soil_vwc": 60,
//     "relay_on_time_s":       2.0,
//     "ambient_temperature":   {"unit":"°C","value":25.3},  // si DHT habilitado
//     "ambient_humidity":      {"unit":"%","value":60.1}    // y lectura válida
//   }
//
// PARÁMETROS:
//   percent : humedad de suelo en % (0.0–100.0)
//   raw     : valor crudo del ADC (0–1023)
// RETORNO   : String con el JSON completo (incluye "}")
// ================================================================
String buildJsonCore(float percent, int raw) {
  // Estado en inglés (estándar para APIs/JSON y MQTT)
  String estado;
  if      (relayState == WATERING) estado = "WATERING";
  else if (relayState == COOLDOWN) estado = "COOLDOWN";
  else if (percent < ON_THRESHOLD_PERCENT) estado = "DRY";
  else    estado = "WET";
  ...
  json += ",\"unit\":\"%\"";
  json += ",\"value\":" + String(percent, 1) + "}";
  json += ",\"watering\":"           + String(relayState == WATERING ? "true" : "false");
  json += ",\"state\":\""            + estado + "\"";
  json += ",\"last_watered_sec\":"   + String(secsAgo);
  json += ",\"on_threshold_soil_vwc\":" + String(ON_THRESHOLD_PERCENT);
  json += ",\"relay_on_time_s\":"    + String((float)RELAY_ON_TIME_MS / 1000.0f, 1);
#if ENABLE_AMBIENT_SENSOR
  if (!isnan(lastAmbientTempC)) {
    json += ",\"ambient_temperature\":{\"unit\":\"°C\",\"value\":" + String(lastAmbientTempC, 1) + "}";
  }
  if (!isnan(lastAmbientHumPct)) {
    json += ",\"ambient_humidity\":{\"unit\":\"%\",\"value\":" + String(lastAmbientHumPct, 1) + "}";
  }
#endif
  json += "}";
  return json;
...
// Respuesta: el payload canónico devuelto por buildJsonCore().
void handleJson() {
  // "application/json" es el Content-Type estándar para APIs REST
  server.send(200, "application/json", buildJsonCore(lastPercent, lastRaw));
}
...
// La Raspberry Pi (con em_server/services/mqtt_service.py corriendo) recibe
// este mensaje, lo parsea y lo almacena en la base de datos para dashboards.
//
// FORMATO DEL JSON PUBLICADO (canónico):
//   {
//     "soil_humidity"          : {"raw":512,"unit":"%","value":51.5},
...
//     "on_threshold_soil_vwc"  : 35,    ← umbral (%) que activa el relé
//     "relay_on_time_s"        : 1.0,   ← duración de cada ciclo de riego en segundos
//     "ambient_temperature"    : {...}, ← solo si DHT habilitado y lectura válida
//     "ambient_humidity"       : {...}
//   }
//
// ¿QUÉ ES QoS EN MQTT?
...
// ================================================================
void publicarMQTT() {
  // En modo calibración no se publica telemetría al broker (evita
  // guardar lecturas de calibración en la base de datos). La presencia
  // online sí se publica desde reconnectMQTT().
#if CALIBRATION_MODE
  return;
#endif
  ...
  // Construir el JSON canónico con la última lectura.
  // c_str() convierte String de Arduino a cadena C (const char*) que
  // necesita mqtt.publish()
  String json = buildJsonCore(lastPercent, lastRaw);
...
void setup() {
  ...
  Serial.println();
  Serial.println("======================================================");
  Serial.println("[CALIBRACION] MODO CALIBRACION ACTIVO");
  Serial.printf("[CALIBRACION] RAW_DRY actual=%d | RAW_WET actual=%d\n", RAW_DRY, RAW_WET);
  Serial.println("[CALIBRACION] Riego desactivado y sin telemetria MQTT.");
  Serial.println("[CALIBRACION] Sensor EN EL AIRE (seco)  -> anota el RAW MAXIMO para RAW_DRY");
  Serial.println("[CALIBRACION] Sensor EN AGUA (humedo)   -> anota el RAW MINIMO para RAW_WET");
  Serial.println("======================================================");
  Serial.println();
#endif
  ...
  if (strlen(MQTT_SERVER) > 0) {
    ...
    // canónico completo (suelo anidado + metadatos + DHT opcional). El
    // default de 256 bytes puede quedarse justo o corto.
...
void loop() {
...
#if CALIBRATION_MODE
  // ── MODO CALIBRACIÓN: muestreo automático cada CALIBRATION_SAMPLE_MS ──
  // Imprime el RAW del ADC, el % actual y el rango Min/Max acumulado para
  // ajustar RAW_DRY (aire/seco → MAX) y RAW_WET (agua/húmedo → MIN).
  if (now - lastCalibrationSampleMs >= CALIBRATION_SAMPLE_MS) {
    lastCalibrationSampleMs = now;

    lastRaw     = readADC();
    lastPercent = rawToPercent(lastRaw);

    if (lastRaw < calibMinRaw) calibMinRaw = lastRaw;
    if (lastRaw > calibMaxRaw) calibMaxRaw = lastRaw;

    Serial.printf("[CALIBRACION] Raw: %4d | Humedad: %5.1f%% | Min: %4d | Max: %4d\n",
                  lastRaw, lastPercent, calibMinRaw, calibMaxRaw);
  }
#else
  ...
  // En modo calibración updateRelay() solo mantiene el relé desactivado.
